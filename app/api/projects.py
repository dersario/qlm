...
from app.schemas import (
    ...
    ProjectUserAssignment, UserResponse
)
...
async def update_project(
    ...
    project_service = ProjectService(db)
    project = project_service.update_project(project_id=project_id, project_update=project_update)
    if project is None:
        raise HTTPException(status_code=404, detail="Проект не найден")
    return project
...
async def assign_user_to_project(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=400, detail="Пользователь уже назначен на проект")
...
async def remove_user_from_project(
    ...
    project_service = ProjectService(db)
    success = project_service.remove_user_from_project(project_id=project_id, user_id=user_id)
    if not success:
        raise HTTPException(status_code=404, detail="Пользователь не назначен на проект")
